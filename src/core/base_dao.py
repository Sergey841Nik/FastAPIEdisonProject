...
class BaseDao:
    ...
    async def find_one_or_none_by_id(self, data: int) -> Row[Any] | None:
        try:
            ...
            logger.info("Запись в таблице %s с ID %s %s", self.model.name, data, "найдена" if rec else "не найдена")
            return rec
    ...
    async def find_one_or_none(self, data: str) -> Row[Any] | None:
        try:
            query: TextClause = text(f"SELECT * FROM {self.model.name} WHERE email = :data")
            query = query.bindparams(data=data)
            result = await self._session.execute(query)
            rec: Row[Any] | None = result.one_or_none()

            logger.info("Запись в таблице %s с %s %s", self.model.name, data, "найдена" if rec else "не найдена")
            return rec
        except SQLAlchemyError as e:
            logger.error("Ошибка %s при поиске записи с %s", e, data)
